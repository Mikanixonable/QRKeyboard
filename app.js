...
(function () {
  ...
  const {
    getRmqrIndex, rmqrWidthsFor, rmqrVersionOf,
    makeSegButton, makeToggle, makeNote,
    gridDims, splitEvenly,
    captionFontFromScale, truncateToWidth, wrapToLines, wrapMonospace, fitCaptionLines,
    captionFontRange, drawCaptionBand,
    scaleCanvasTo, clampBox, sampleScanColorsDual, sampleScanColors,
  } = AppUtil;
      ...
      if (scanVideo.videoWidth > 0) {
        ...
        try {
          ...
          if (frameCount % 3 === 0) {
            ...
            const found = FinderLib.tryDecodeMicroRmqr(small);
            if (found) {
              stopCameraScan();
  ...
  function decodeImageFile(file) {
    ...
    img.onload = () => {
      ...
      try {
        ...
        const found = FinderLib.tryDecodeMicroRmqr(off);
        if (found) {
